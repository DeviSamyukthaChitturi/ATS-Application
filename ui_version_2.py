...
def preserve_and_clean_entities(text):
    ...
    doc = nlp(text)  # `nlp` should be a spaCy model, e.g., spacy.load("en_core_web_sm")
...
if submit_button:
    if uploaded_file is not None:
        ...
        st.write("Resume and Jd breakdown")
        st.write(input_df.head())
         # Provide a download button for the dataset
        result_csv = input_df.to_csv(index=False)
        st.download_button(
            label="Download combined resue and JD",
            data=result_csv,
            file_name="Resume_jd_combined.csv",
            mime="text/csv"
        )
